...
Автоматическая коррекция цветопередачи (кислотный зелёный) на ноутбуках при снижении яркости дисплея.
...
Некоторые панели ноутбуков (например CSO SNE007ZA2-1 от China Star Optoelectronics) при снижении яркости подсветки дают заметный сдвиг цветопередачи — зелёный становится кислотным, неестественным. Это аппаратная особенность панели, проявляется и на Linux, и на Windows.

## Решение

Скрипт отслеживает изменение яркости и автоматически корректирует гамму зелёного канала через `xrandr`:

- **Максимальная яркость** — без коррекции
- **75%–100%** — плавное нарастание коррекции
- **Ниже 75%** — полная коррекция
...
3. Спросит силу коррекции (по умолчанию 0.75)
...
```bash
# Попробовать коррекцию (замените eDP на ваш дисплей)
xrandr --output eDP --gamma 1.0:0.85:1.0   # слабая
xrandr --output eDP --gamma 1.0:0.75:1.0   # средняя
xrandr --output eDP --gamma 1.0:0.65:1.0   # сильная

# Сбросить
xrandr --output eDP --gamma 1.0:1.0:1.0
```

Формат: `R:G:B`. Значение зелёного (G) меньше 1.0 уменьшает зелёный канал.
...
Отредактируйте `~/.local/bin/brightness-gamma-fix.sh` — задайте значения `DISPLAY_NAME`, `BACKLIGHT`, `MAX`, `MID`, `MIN_GAMMA` под свою систему.
...
| `MIN_GAMMA` | `0.75` | Сила коррекции (меньше = сильнее) |
...
3. Скрипт читает текущую яркость и вычисляет коэффициент коррекции зелёного канала
